...
class ResultsTabTabMixin:
    ...
    def _tab_results(self, root):
        ...
        if not hasattr(app, "var_auto_legend"):
            app.var_auto_legend = tk.BooleanVar(value=True)
        if not hasattr(app, "var_legend_regex"):
            app.var_legend_regex = tk.StringVar(
                value=r"(?:^|_)"
                      r"(?P<area>BR_AR_Paraguay|SouthAmerica)"
                      r"_Sys3_(?P<alt>340|525)km"
                      r"_FS(?P<fs>20|40)m"
                      r"_LF(?P<lf>10|20|50)"
                      r"_M(?P<margin>\d+)km"
                      r"_Azi(?P<azi>\d+)deg"
        ...
        if not hasattr(app, "var_legend_template"):
            app.var_legend_template = tk.StringVar(value="h={fs}m, azi={azi}deg,lf={lf}%,M={margin}km")
        if not hasattr(app, "var_legend_value_map"):
            app.var_legend_value_map = tk.StringVar(value="BR_AR_Paraguay=PA,SouthAmerica=SA")
        ...
        # Automatic legend from folder names.
        frm_auto_leg = ttk.LabelFrame(left, text="Legenda automatica")
        frm_auto_leg.pack(fill="x", pady=(0, 6))
        ttk.Checkbutton(
            frm_auto_leg,
            text="Usar quando Legenda manual estiver vazia",
            variable=app.var_auto_legend,
            command=app._draw_results_plots,
        ).grid(row=0, column=0, columnspan=2, padx=4, pady=(4, 2), sticky="w")

        ttk.Label(frm_auto_leg, text="Regex:").grid(row=1, column=0, padx=4, pady=2, sticky="w")
        ent_legend_regex = ttk.Entry(frm_auto_leg, textvariable=app.var_legend_regex, width=34)
        ent_legend_regex.grid(row=1, column=1, padx=4, pady=2, sticky="we")

        ttk.Label(frm_auto_leg, text="Template:").grid(row=2, column=0, padx=4, pady=2, sticky="w")
        ent_legend_template = ttk.Entry(frm_auto_leg, textvariable=app.var_legend_template, width=34)
        ent_legend_template.grid(row=2, column=1, padx=4, pady=2, sticky="we")

        ttk.Label(frm_auto_leg, text="Mapeamento:").grid(row=3, column=0, padx=4, pady=2, sticky="w")
        ent_legend_map = ttk.Entry(frm_auto_leg, textvariable=app.var_legend_value_map, width=34)
        ent_legend_map.grid(row=3, column=1, padx=4, pady=2, sticky="we")

        def _redraw_on_legend_config(*_):
            app._draw_results_plots()

        ent_legend_regex.bind("<Return>", _redraw_on_legend_config)
        ent_legend_template.bind("<Return>", _redraw_on_legend_config)
        ent_legend_map.bind("<Return>", _redraw_on_legend_config)
        ttk.Button(frm_auto_leg, text="Atualizar legendas", command=app._draw_results_plots).grid(
            row=4, column=0, columnspan=2, padx=4, pady=(2, 4), sticky="we"
        )
        frm_auto_leg.grid_columnconfigure(1, weight=1)
    ...
    def _parse_legend_value_map(self) -> dict[str, str]:
        raw = ""
        try:
            raw = self.var_legend_value_map.get()
        except Exception:
            return {}
    ...
    def _auto_legend_for_folder(self, folder_tag: str) -> str:
        raw = folder_tag[6:] if folder_tag.startswith("ssh://") else folder_tag
        folder_name = Path(raw).name

        try:
            regex = self.var_legend_regex.get().strip()
            template = self.var_legend_template.get().strip()
        except Exception:
            return folder_name
        ...
        try:
            match = re.search(regex, folder_name)
        except re.error:
            return folder_name
        ...
        value_map = self._parse_legend_value_map()
        for key, value in list(values.items()):
            values[key] = value_map.get(value, value)
    ...
    def _legend_label_for_folder(self, folder_tag: str) -> str:
        st = self._dir_style.get(folder_tag, {})
        manual_label = (st.get("label") or "").strip()
        if manual_label:
            return manual_label

        try:
            use_auto = bool(self.var_auto_legend.get())
        except Exception:
            use_auto = False

        if use_auto:
            return self._auto_legend_for_folder(folder_tag)
    ...
    def _draw_results_plots(self):
        ...
        for i in range(n_axes):
            ...
            for folder_tag in dirs:
                ...
                st = self._dir_style.get(folder_tag, {})
                label = self._legend_label_for_folder(folder_tag)
